...
# -*- coding: utf-8 -*-
"""
Phase-2 OCT2017 PDF packer (A4-friendly, precision-trimmed tables)

- Metrics + calibration + reliability curves
- Robustness & Efficiency as paginated ReportLab tables
- Numbers formatted to fixed precision (default 4 decimals)
- TensorBoard scalars/images (robust API use)
- Dataset contact sheets (train/val/test)
"""
...
def load_csv_header_rows(p: Path):
    """Return (header, rows). Empty if not found."""
...
# -------------------------
# Table formatting (precision + widths)
# -------------------------
...
def _table_style(base_font=8, numeric_cols: List[int] = None):
    ...
    # Right-align numeric columns for cleanliness
    if numeric_cols:
        for col in numeric_cols:
            st.add("ALIGN", (col,1), (col,-1), "RIGHT")
    # Left-align first column (usually label)
    st.add("ALIGN", (0,1), (0,-1), "LEFT")
    return st

def _desired_widths_for(header: List[str], avail_w: float) -> List[float]:
    """
    Heuristic column widths in cm converted to points, then scaled to fit avail_w.
    If header matches robustness-style columns, we give wider metric columns.
    """
    h = [x.strip().lower() for x in header]
    # Baseline cm widths by name (fallback=2.0)
    base_cm = []
    for name in h:
    ...
    # Convert to points and scale to fit
...
def _draw_paginated_table(c: canvas.Canvas, title: str, header: List[str], rows: List[List[str]],
    ...
    """
    Draw a table across as many pages as needed. Values formatted to 'decimals'.
    """
    # available rectangle for table
    avail_w = page_w - 2*side_margin
    avail_h = page_h - top_margin - bottom_margin

    # Normalize header width and build numeric column index
    ...
    while idx < n or (idx==0 and n==0):
        ...
        # greedily add rows until height exceeds avail_h
        data_page = [header]
        j = idx
        while j < n:
            ...
            if h > avail_h:  # too tall; remove last row and stop
                data_page.pop()
                break
            j += 1

        # if no rows fit besides header, force one row per page
    ...
    if cal:
        ...
        for split in ["val","test"]:
            if split in cal:
                ...
                draw_kv(c, 2*cm, y, f"{split.Upper() if hasattr(str,'Upper') else split.upper()} NLL raw/cal", f"{_fmt4(d.get('nll_raw','-'))} / {_fmt4(d.get('nll_cal','-'))}"); y -= 0.6*cm
...
def add_pages_robust_eff(c: canvas.Canvas, title: str, dir_path: Path, page_w, page_h):
    # Robustness table (paginated, 4 decimals)
    ...
    # Efficiency tables (each paginated independently)
...
def main():
    ...
    ap.add_argument("--tb-root-list", default="", help="Comma-separated TB log roots")
